...
import RabbitInstance from '@rabbitmq/RabbitInstance';
...
export default class RabbitConsumer {
  ...
  private sendBackResponse = (consumer: string, message: ConsumeMessage, content: object): boolean => {
    ...
    console.log(`[${correlationId}] Sending back the response to the queue '${message.properties.replyTo}' as ${consumer}.`);
  ...
  protected consumeIncomingMessage = async (
    ...
    const { consumerTag } = await this.incomingChannel.consume(incomingQueue, (message) => {
      ...
      console.log(`[${correlationId}] Consuming an incoming message on the queue '${incomingQueue}'.`);
      ...
      messageCallback(message.content.toString()).then((response) => {
        console.log(`[${correlationId}] Consumed an incoming message on the queue '${incomingQueue}'.`);
        this.sendBackResponse(consumerTag!, message, Buffer.from(JSON.stringify(response)));
      }).catch((err: Error | unknown) => {
        console.log(`[${correlationId}] Could not consume an incoming message on the queue '${incomingQueue}': ${(err as Error).message}`);
      }).finally(() => {
